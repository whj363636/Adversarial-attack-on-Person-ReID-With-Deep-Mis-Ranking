...
parser.add_argument('--loss', type=str, default='cent', choices=['cent', 'xent', 'htri', 'xent_htri'])
...
def main(opt):
  ...
  if args.mode == 'test':
    epoch = 'test'
    test(G, test_target_net, dataset, queryloader, galleryloader, epoch, use_gpu, is_test=True)
    return 0
  ...
  for epoch in range(1,args.epoch+1):
    ...
    train(epoch, G, target_net, clf_criterion, metric_criterion, optimizer_G, trainloader, use_gpu)
    train_time += round(time.time() - start_train_time)

    if epoch % args.eval_freq == 0:
      print("==> Eval at epoch {}".format(epoch))
      if args.ak_type < 0:
        cmc, mAP = test(G, test_target_net, dataset, queryloader, galleryloader, epoch, use_gpu, is_test=False)
        ...
        all_hits = test(G, target_net, dataset, queryloader, galleryloader, epoch, use_gpu, is_test=False)
...
def train(epoch, G, target_net, clf_criterion, metric_criterion, optimizer_G, trainloader, use_gpu):
  G.train()
  global is_training
  is_training = True
  ...
  for batch_idx, (imgs, pids, _, pids_raw) in enumerate(trainloader):
    ...
    new_imgs, mask = perturb(imgs, G, train_or_test='train')
    ...
    if (batch_idx+1) % args.print_freq == 0:
      print("===> Epoch[{}]({}/{}) loss: {:.4f}".format(epoch, batch_idx, len(trainloader), loss_G.item()))

def test(G, target_net, dataset, queryloader, galleryloader, epoch, use_gpu, is_test=False, ranks=[1, 5, 10, 20]):
  global is_training
  is_training = False
  if args.mode == 'test' and args.G_resume_dir:
    G_resume_dir = args.G_resume_dir
    G_checkpoint = torch.load(G_resume_dir)
    ...
    print("Sucessfully, loading {}".format(G_resume_dir))

  with torch.no_grad():
    qf, lqf, new_qf, new_lqf, q_pids, q_camids = extract_and_perturb(queryloader, G, target_net, use_gpu, query_or_gallery='query', is_test=is_test, epoch=epoch)
    gf, lgf, g_pids, g_camids = extract_and_perturb(galleryloader, G, target_net, use_gpu, query_or_gallery='gallery', is_test=is_test, epoch=epoch)
...
def extract_and_perturb(loader, G, target_net, use_gpu, query_or_gallery, is_test, epoch):
  ...
  for batch_idx, (imgs, pids, camids, pids_raw) in enumerate(loader):
    ...
    if query_or_gallery == 'query':
      G.eval()
      new_imgs, delta, mask = perturb(imgs, G, train_or_test='test')
...
def perturb(imgs, G, train_or_test='test'):
  n,c,h,w = imgs.size()
  mask = None
